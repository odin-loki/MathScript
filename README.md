...
- **Tests:** **430** CTest suites, 100% passing (CUDA disabled in CI); ~91% line coverage (**90%** minimum enforced in CI)
...
## Product closure (feature waves 231–271)

After the profiling iteration closed at Wave 230, forty-one parallel **feature waves** (231–271) expanded crypto, FEM/CFD, distributed stubs, symbolic transforms, REPL bindings, and Qt GUI polish. Wave 271 added FEM/CFD 3D composable primitives, Weierstrass zeta/sigma, extra Jacobi ratios, GRIA GF(2ⁿ) field enumeration, quantum eigenspectrum/ground state, and `assign_matrix_call_tail10`. See [`CHANGELOG.md`](CHANGELOG.md) for per-wave detail. Remaining roadmap depth (scalable multi-node MPI linear algebra, full IDE/LSP) stays deferred per [`mathscript-master-plan.md`](mathscript-master-plan.md) §2.12/§7/§10/§11.
