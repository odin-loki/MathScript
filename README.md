...
- **Tests:** **432** CTest suites, 100% passing (CUDA disabled in CI); ~91% line coverage (**90%** minimum enforced in CI)
...
## Product closure (feature waves 231–273)

After the profiling iteration closed at Wave 230, forty-three parallel **feature waves** (231–273) expanded crypto, FEM/CFD, distributed stubs, symbolic transforms, REPL bindings, and Qt GUI polish. Wave 273 added izaac VRF prove/verify, encrypt/decrypt, randn matrix, quantum Schmidt number/tensor/outer, Cypha NIG moments, `theta1_prime`/`jacobi_theta`, and `cfd_run_advection_3d`. See [`CHANGELOG.md`](CHANGELOG.md) for per-wave detail. Remaining roadmap depth (scalable multi-node MPI linear algebra, full IDE/LSP) stays deferred per [`mathscript-master-plan.md`](mathscript-master-plan.md) §2.12/§7/§10/§11.
