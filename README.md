...
- **Tests:** **427** CTest suites, 100% passing (CUDA disabled in CI); ~91% line coverage (**90%** minimum enforced in CI)
...
## Product closure (feature waves 231–268)

After the profiling iteration closed at Wave 230, thirty-eight parallel **feature waves** (231–268) expanded crypto, FEM/CFD, distributed stubs, symbolic transforms, REPL bindings, and Qt GUI polish. Wave 268 added extended ML/PDE/optim/compress REPL bindings and Kebab Case (Ctrl+Alt+K). See [`CHANGELOG.md`](CHANGELOG.md) for per-wave detail. Remaining roadmap depth (scalable multi-node MPI linear algebra, full IDE/LSP) stays deferred per [`mathscript-master-plan.md`](mathscript-master-plan.md) §2.12/§7/§10/§11.
