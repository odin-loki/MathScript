...
## Project Status: v1.0.0 — Phase 10 (Hardening) + Wave 202 Complete
...
- **Wave 195:** REPL scalar bindings batch 2 — 20 more previously library-only functions exposed to the interactive shell from `ms::special` (`erfinv`, `erfcinv`, `log_gamma`, `digamma`, `trigamma`, `polygamma`, `gamma_inc_reg`, `gamma_inc_reg_upper`, `beta_inc_reg`), `ms::prob` (`uniform_pdf`, `t_pdf`, `t_ppf`, `gamma_ppf`, `beta_ppf`, `f_pdf`, `f_ppf`), and `ms::finance` (`mc_lookback_floating_call`/`_put`, `mc_lookback_fixed_call`/`_put` — the Wave 194 lookback pricers), all using existing regex-literal-dispatch tiers with no new arity tier needed; `ms::finance` gains the Black-Litterman model — `bl_implied_returns` (reverse-optimization prior, `Pi = delta*Sigma*w_mkt`), `bl_posterior_returns` (full closed-form posterior blending prior and investor views via `[(tau*Sigma)^-1 + P^T*Omega^-1*P]^-1 * [(tau*Sigma)^-1*Pi + P^T*Omega^-1*Q]`), and `bl_posterior_returns_default_omega` (convenience overload deriving the standard diagonal view-uncertainty matrix), built on a generalized multi-RHS extension of the existing Gaussian-elimination solver and verified against a hand-computed closed-form reference case; `ms::graph` gains Louvain community detection — `louvain` (two-phase greedy modularity optimization: local vertex-moving then community aggregation, repeated to convergence) and `modularity` (Newman-Girvan weighted modularity), verified via disjoint-clique recovery, bridged-cluster recovery, and a case where a known modularity resolution-limit effect was correctly identified rather than mistaken for a bug. 365 suites (no new CTest registrations; 60 new test cases across existing `test_repl_commands`/`test_finance`/`test_graph` binaries).
- **Wave 196:** `ms::finance` gains closed-form geometric-average Asian options (`geo_asian_call`/`geo_asian_put`, Kemna-Vorst formula on a synthetic Black-76 forward, verified via exact reduction to Black-Scholes at `n_fixings=1`, convergence to the continuous closed form as `n_fixings→∞`, and a Monte Carlo geometric-mean cross-check) and `trinomial_option` (Boyle's trinomial tree, verified against Black-Scholes convergence, the American-put early-exercise premium, and the existing binomial-tree pricer); `ms::poly` gains `poly_rational_roots`/`poly_factor_rational` (Rational Root Theorem-based factorization over Q for near-integer-coefficient polynomials, built on `numthy::divisors`, verified via full round-trip reconstruction and independent root re-evaluation); `ms::signal` gains generic `filter`/`filtfilt` (direct-form difference-equation application and zero-phase double-pass filtering) and `firwin`/`firwin_highpass` (windowed-sinc FIR filter design with Rectangular/Hamming/Hann/Blackman windows), verified via amplitude-response and complementary-filter identity checks. 365 suites (no new CTest registrations; 68 new test cases across existing `test_finance`/`test_poly_ext`/`test_signal_filters` binaries).
- **Wave 197:** `ms::special` gains the Voigt profile (`voigt`, Gaussian-Lorentzian convolution via a from-scratch Humlicek (1982) w4 rational approximation to the Faddeeva function — no complex-error-function implementation previously existed in the codebase) and `pseudo_voigt`/`pseudo_voigt_auto` (Thompson-Cox-Hastings shared-FWHM Gaussian/Lorentzian mixing approximation), verified primarily via Simpson's-rule normalization integrals (≈1.0 across parameter regimes) plus Gaussian/Lorentzian limiting-case convergence; `ms::stats` gains the Friedman test (`friedman`, the non-parametric repeated-measures alternative to one-way ANOVA, with tie-corrected rank-sum chi-square statistic reusing the existing `average_ranks` helper and `chi2_cdf`), verified against a hand-computed textbook-style reference case matching to 1e-9; `ms::tensorops` gains Tensor-Train decomposition (`decompose_tt`/`reconstruct_tt`, the TT-SVD algorithm for compressing tensors of order 4+ where Tucker's core grows exponentially, with per-step error-budget splitting per Oseledets 2011), which surfaced and worked around a pre-existing bug in the shared `ms::linalg::svd` routine (wrong reconstruction for rank-deficient matrices) via a self-contained Jacobi SVD, verified via exact rank-1/low-rank recovery and monotonic reconstruction-error-vs-epsilon behavior. 366 suites (1 new CTest registration: `test_special_voigt`; 53 new test cases across `test_special_voigt`/`test_stats_prob_ext`/`test_tensorops`).
- **Wave 198:** fixed two real bugs in the shared `ms::linalg::svd` LAPACK-style kernels, found while investigating the rank-deficient-reconstruction issue flagged in Wave 197 — (1) `dorgbr`'s `vect='P'`/tall-matrix branch called the Householder-reflector accumulator with the wrong transpose flag (`apply_pt=true` instead of `false`), corrupting every right singular vector beyond the first reflector's span for any `m>=n` input with `n>=3` (not just rank-deficient ones — rank deficiency just made it visually obvious); (2) `dlartg`'s Givens-rotation sign computation (`r = g/copysign(sn,g)`) had the wrong sign whenever the larger-magnitude input was negative, breaking `U`/`V` orthogonality while leaving reconstruction looking self-consistent — fixed via direct `r=hypot(f,g)`, `cs=f/r`, `sn=g/r`. Verified via column-by-column reconstruction, `U^T U`/`V^T V ≈ I` orthogonality checks across rank-1/2/half-rank square/tall/wide matrices, full-rank regression, and `pinv`/`null`/`orth` regressions — all of `test_linalg_*`, `test_blas_lapack`, and `test_tensorops` pass, and the full 367-suite run confirms no other regressions from this shared-kernel fix; `ms::quantum` gains Wigner-function and Husimi-Q phase-space quasi-probability distributions (`wigner_function`, `husimi_Q`, built on the existing truncated-Fock-basis `coherent_state` infrastructure via the Cahill-Glauber displaced-parity trace formula and a generic matrix exponential), verified via Husimi normalization/positivity and — the canonical non-classicality "smoking gun" test — exact Wigner-function negativity at the origin for the Fock `|1⟩` state (`-1/π`, matching the textbook closed form); `ms::prob` gains `lognormal`/`weibull`/`laplace` pdf/cdf/ppf trios (Weibull and Laplace via closed-form quantile functions, lognormal via the existing normal quantile through a log transform), verified via round-trip `ppf∘cdf`/`cdf∘ppf` identities and exact closed-form relationships (Weibull `k=1` ≡ Exponential, lognormal median `=exp(mu)`, Weibull `cdf(lambda)=1-e^-1` regardless of shape). 367 suites (1 new CTest registration: `test_prob_lognormal_weibull_laplace`; 65 new test cases across `test_linalg_decomp`/`test_quantum`/the new prob test binary).
- **Wave 199:** `ms::info` gains `blahut_arimoto`/`channel_capacity` (iterative Blahut-Arimoto algorithm for the capacity of any discrete memoryless channel given its transition matrix, plus the optimal input distribution achieving it), verified against the closed-form BSC/BEC capacities, noiseless-channel (`log2(n)`) and useless-channel (`~0`) sanity limits, and general non-negativity/upper-bound checks; `ms::topo` gains `cech_complex` (Čech complex construction from a point-cloud distance matrix — vertices always included, edges via the `dist<=2*epsilon` ball-intersection rule, triangles via true minimum-enclosing-ball radius from Heron's-formula circumradius rather than the flag-complex/all-edges-present shortcut Vietoris-Rips uses, since Čech is not a flag complex), verified via a 3-4-5 right-triangle circumradius exact check, an equilateral-triangle threshold test, and a unit-square diagonal case demonstrating the Čech/Vietoris-Rips divergence at the 2-simplex level; `ms::control` gains `gram`/`ctrb_gram`/`obsv_gram` (continuous-time controllability/observability Gramians, computed via the existing `lyap` solver applied to `A*Wc+Wc*A^T+B*B^T=0` and `A^T*Wo+Wo*A+C^T*C=0`), verified via symmetry, positive-semi-definiteness (eigenvalues), direct Lyapunov-equation-residual substitution, and an exact closed-form scalar-plant check (`Wc=b^2/(2a)`). 367 suites (no new CTest registrations — all tests added to the existing `test_info`/`test_topo`/`test_control` binaries; 32 new test cases total).
- **Wave 200:** `ms::cplx` gains `green_function_disk(z, z0, radius=1.0)` (the Dirichlet Green's function for the Laplacian on a disk, `G(z,z0)=(1/2*pi)*ln|(z-z0)/(1-conj(z0)*z)|` rescaled by `radius`), verified via symmetry, hand-derived known values, boundary vanishing, the `z->z0` singularity, and — the deepest cross-check — a finite-difference confirmation that its boundary normal derivative reproduces the existing `poisson_kernel`; `ms::optim` gains `illinois(f, a, b, tol, max_iter)` (the Illinois anti-stagnation modification of regula falsi, which halves the retained endpoint's function value after two consecutive retentions to avoid the classic false-position stagnation failure mode), verified against known roots, a classic stagnation-prone bracket with a direct iteration-count improvement over plain regula falsi, agreement with `brentq`/`bisection`, and non-bracketing/root-at-endpoint edge cases; `ms::combo` gains `bracelets(n, k)` (necklaces up to the full dihedral group — rotation AND reflection — rather than just cyclic rotation as `necklaces` uses), verified via the `bracelets(n,k) <= necklaces(n,k)` invariant, no-duplicate-equivalence-class and dihedral-closure checks, and known small-case counts. 367 suites (no new CTest registrations — all tests added to the existing `test_cplx`/`test_optim_global`/`test_combo` binaries; 25 new test cases total).
- **Wave 201:** `ms::ml` gains `ElasticNet` (combined L1+L2 penalized regression, `alpha*(l1_ratio*||w||_1 + 0.5*(1-l1_ratio)*||w||_2^2)`, via the same cyclic-coordinate-descent structure as the existing `LassoRegression` generalized to a modified soft-threshold update), verified via its `l1_ratio=1.0`/`l1_ratio=0.0` limiting agreement with `LassoRegression`/`RidgeRegression`, sparsity-pattern sanity checks at intermediate mixing, and recovery of known coefficient signs/magnitudes on a synthetic regression problem with irrelevant noise features; `ms::diffgeo` gains `gauss_bonnet_integral`/`gauss_bonnet_residual` (numerical verification of `∬K dA = 2*pi*chi` via composite-trapezoid quadrature over the existing `first_fundamental_form`/`gaussian_curvature` machinery), verified on a unit sphere (`->4*pi`, `chi=2`), scale-invariance across non-unit-radius spheres, a flat patch (`->0`), and monotonic grid-refinement convergence; `ms::ode` gains `ode_cashkarp` (the classic 6-stage embedded Cash-Karp Runge-Kutta (4,5) pair with adaptive step-size control, mirroring `ode_rk45`'s exact accept/reject/rescale control-loop structure), verified against exact exponential decay/growth solutions, agreement with `ode_rk45` on a smooth nonlinear IVP, and adaptivity sanity checks (tighter tolerance yields more/smaller steps). 367 suites (no new CTest registrations — all tests added to the existing `test_ml`/`test_diffgeo`/`test_ode_adaptive` binaries; 28 new test cases total).
- **Wave 202** (largest single wave yet — 6 parallel subagents across 6 modules): `ms::fft` gains `goertzel` (O(n) single-frequency-bin DFT extraction via the classic 2-pole IIR recurrence, verified against full `fft()` and a naive DFT — the subagent caught and fixed an incorrect finalization formula in its own task brief before shipping); `ms::pde` gains `pde_advection_1d_lax_wendroff` (second-order-accurate finite-difference advection, verified to have less than half the RMSE of the existing first-order-upwind `pde_advection_1d` on a smooth Gaussian pulse); `ms::compress` gains `wavelet_compress`/`wavelet_decompress` (single-level Haar DWT with hard-thresholded detail coefficients for lossy compression, verified lossless at threshold=0 and with graceful quality degradation as threshold increases); `ms::bignum` gains `bigint_divmod` (combined quotient+remainder for `BigInt`, via a shared-helper refactor of `operator/`/`operator%` that eliminates their previous redundant double-computation of the long-division loop, verified to preserve all pre-existing operator behavior exactly); `ms::izaac` gains `fuzz::mutate` (deterministic, CSPRNG-seeded bounded byte-buffer mutation — flip/insert/delete/splice — for fuzz-corpus seed expansion); `ms::image` gains `adapthisteq` (CLAHE: tile-local clip-limited histogram equalization with bilinear inter-tile blending, verified to out-perform global `histeq` on locally-low-contrast synthetic images). 367 suites (no new CTest registrations — all tests added to existing binaries; 43 new test cases total).
...
- **Unit tests:** 367 CTest suites — all passing (CUDA disabled)
