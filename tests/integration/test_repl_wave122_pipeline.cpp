// MathScript Integration Tests: REPL Interpreter – Wave 63–122 Bindings Pipeline
...
TEST(ReplWave122Pipeline, Wave63_122_BindingsPipeline) {
    Interpreter interp;
    ...
    // --- Wave 122 REPL bindings (single session) ---

    // 154. control_lqr BasicLQR -> K is 1x2
    expect_ok(interp, "Q2 = eye(2)");
    expect_ok(interp, "Klqr = control_lqr([-2, 1; 0, -3], [1; 1], Q2, [1])");
    ASSERT_GT(interp.state().matrices.count("Klqr"), 0u);
    const auto& Klqr = interp.state().matrices.at("Klqr");
    EXPECT_EQ(Klqr.rows(), 1u);
    EXPECT_EQ(Klqr.cols(), 2u);

    // 155. combo_rank_combination v=[0;1],n=4->0; v=[0;2],n=4->1
    expect_ok(interp, "crc0 = combo_rank_combination([0; 1], 4)");
    EXPECT_NEAR(interp.state().scalars.at("crc0"), 0.0, 1e-9);
    expect_ok(interp, "crc1 = combo_rank_combination([0; 2], 4)");
    EXPECT_NEAR(interp.state().scalars.at("crc1"), 1.0, 1e-9);

    // 156. lz77_encode_vec / lz77_decode_vec roundtrip abcabcabc
    ...
    ASSERT_GT(interp.state().matrices.count("lzR"), 0u);
    const auto& lzR = interp.state().matrices.at("lzR");
    EXPECT_EQ(lzR.rows(), 9u);
    EXPECT_NEAR(lzR(0, 0), 97.0, 1e-9);
    EXPECT_NEAR(lzR(8, 0), 99.0, 1e-9);
}
