// MathScript Integration Tests: REPL Interpreter – Wave 63–120 Bindings Pipeline
...
TEST(ReplWave120Pipeline, Wave63_120_BindingsPipeline) {
    Interpreter interp;
    ...
    EXPECT_EQ(interp.state().matrices.at("lzwR").rows(), 6u);
    ...
    // --- Wave 120 REPL bindings (single session) ---

    // 149. huffman_encode_vec / huffman_decode_vec roundtrip on 6-byte test vector
    ...
    const auto& huf_restored = interp.state().matrices.at("hufR");
    EXPECT_EQ(huf_restored.rows(), 6u);
    const double huf_expected[] = {97, 98, 99, 97, 97, 98};
    for (size_t i = 0; i < 6; ++i) {
        EXPECT_NEAR(huf_restored(i, 0), huf_expected[i], 1e-9);
    }

    // 150. geo_volume_tetrahedron unit tetrahedron -> 1/6
