...
TEST(IntegrationFem, FemLoad3dCfdGrid3d_FemMesh3dBoxFemLoad3dPipeline2) {
    Interpreter interp;
...
TEST(IntegrationFem,  KelvinBeiScalar) {
    Interpreter interp;
    expect_ok(interp, "kbi = kelvin_bei(0, 1)");
    EXPECT_NEAR(interp.state().scalars.at("kbi"), ms::kelvin_bei(0, 1.0), 1e-8);
}
