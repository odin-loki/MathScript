...
TEST(IntegrationQuantum,  BfgsMinimizesQuadratic) {
    Interpreter interp;
    expect_near_in_output(interp, R"cmd(bfgs("(x0-3)*(x0-3)", [0]))cmd", 3.0, 1e-3);
}

TEST(IntegrationQuantum,  AdvancedOptimBindings) {
    Interpreter interp;
...
TEST(IntegrationQuantum,  ControlAnalysisBindings) {
    Interpreter interp;
...
TEST(IntegrationQuantum,  QuantumInfoBindings) {
    Interpreter interp;
