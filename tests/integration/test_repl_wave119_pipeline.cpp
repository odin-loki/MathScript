// MathScript Integration Tests: REPL Interpreter – Wave 63–119 Bindings Pipeline
...
TEST(ReplWave119Pipeline, Wave63_119_BindingsPipeline) {
    Interpreter interp;
    ...
    // --- Wave 119 REPL bindings (single session) ---

    // 146. lzw_encode_vec / lzw_decode_vec roundtrip on abcabc bytes
    ...
    const auto& lzw_restored = interp.state().matrices.at("lzwR");
    EXPECT_EQ(lzw_restored.rows(), 6u);
    const double lzw_expected[] = {97, 98, 99, 97, 98, 99};
    for (size_t i = 0; i < 6; ++i) {
        EXPECT_NEAR(lzw_restored(i, 0), lzw_expected[i], 1e-9);
    }

    // 147. quantum_coherent_state(1,0,20) -> 21x1 unit-norm column
    expect_ok(interp, "coh = quantum_coherent_state(1, 0, 20)");
    ASSERT_GT(interp.state().matrices.count("coh"), 0u);
    EXPECT_EQ(interp.state().matrices.at("coh").rows(), 21u);
    double coh_norm_sq = 0.0;
    for (size_t i = 0; i < 21; ++i) {
        const double v = interp.state().matrices.at("coh")(i, 0);
        coh_norm_sq += v * v;
    }
    EXPECT_NEAR(coh_norm_sq, 1.0, 1e-5);

    // 148. geo_dist_point_line2d(0,0,1,1,-1) -> sqrt(2)/2
    expect_ok(interp, "dpl = geo_dist_point_line2d(0, 0, 1, 1, -1)");
    EXPECT_NEAR(interp.state().scalars.at("dpl"), std::sqrt(2.0) / 2.0, 1e-5);
}
