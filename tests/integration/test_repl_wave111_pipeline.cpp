// MathScript Integration Tests: REPL Interpreter – Wave 63–111 Bindings Pipeline
...
TEST(ReplWave111Pipeline, Wave63_111_BindingsPipeline) {
    Interpreter interp;
    ...
    // --- Wave 110 REPL bindings (single session) ---

    // 125. geo_point_in_polygon (see test_geo GeoIntersect.PointInPolygon)
    expect_ok(interp, "sq = [0, 0; 4, 0; 4, 4; 0, 4]");
    expect_ok(interp, "pip = geo_point_in_polygon(2, 2, sq)");
    EXPECT_NEAR(interp.state().scalars.at("pip"), 1.0, 1e-9);

    // 126. ml_categorical_crossentropy (see test_ml MLLoss.CategoricalCrossentropy)
    expect_ok(interp, "pred_c = [0.7, 0.2, 0.1; 0.1, 0.8, 0.1]");
    expect_ok(interp, "true_c = [1, 0, 0; 0, 1, 0]");
    expect_ok(interp, "cce = ml_categorical_crossentropy(pred_c, true_c)");
    EXPECT_GT(interp.state().scalars.at("cce"), 0.0);
    EXPECT_LT(interp.state().scalars.at("cce"), 1.0);
    EXPECT_TRUE(std::isfinite(interp.state().scalars.at("cce")));

    // 127. geo_overlap_circles (same center radius 1 -> overlap)
    expect_ok(interp, "ov = geo_overlap_circles(0, 0, 1, 0, 0, 1)");
    EXPECT_NEAR(interp.state().scalars.at("ov"), 1.0, 1e-9);

    // --- Wave 111 REPL bindings (single session) ---

    // 128. geo_bezier_eval_x (see test_geo GeoBezier.Eval ctrl [[0,0],[1,2],[2,0]] t=0.5 -> x=1)
    expect_ok(interp, "ctrl = [0, 0; 1, 2; 2, 0]");
    expect_ok(interp, "bx = geo_bezier_eval_x(ctrl, 0.5)");
    EXPECT_NEAR(interp.state().scalars.at("bx"), 1.0, 1e-9);

    // 129. geo_bezier_eval_y (see test_geo GeoBezier.Eval t=0.5 -> y=1)
    expect_ok(interp, "by = geo_bezier_eval_y(ctrl, 0.5)");
