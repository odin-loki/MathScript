...
TEST(IntegrationGraph,  SpectrumLaplacianTail31) {
    Interpreter interp;
    expect_contains(interp, "help", "graph_adjacency_spectrum(A)");
    expect_contains(interp, "help", "graph_laplacian(A)");
...
TEST(IntegrationGraph,  WeberEScalar) {
    Interpreter interp;
