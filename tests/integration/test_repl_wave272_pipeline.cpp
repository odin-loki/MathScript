// MathScript Integration Tests: REPL Interpreter – Wave 272 Pipeline
//
// Wave 272 REPL smoke: CFD mass 3D, ellip_d, quantum Schmidt/anticommutator, izaac MPC/backtest/VRF/fuzz,
// axiom GRIA/evolve, GRIA dispatch hints, FEM 3D composable pipeline.
...
TEST(ReplWave272Pipeline, CfdIntegratedMass3d) {
    Interpreter interp;
...
TEST(ReplWave272Pipeline, QuantumAndSpecial) {
    Interpreter interp;
...
TEST(ReplWave272Pipeline, IzaacFrameworks) {
    Interpreter interp;
...
TEST(ReplWave272Pipeline, AxiomGriaFem3d) {
    Interpreter interp;
