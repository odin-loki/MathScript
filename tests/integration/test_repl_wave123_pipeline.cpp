// MathScript Integration Tests: REPL Interpreter – Wave 63–123 Bindings Pipeline
...
TEST(ReplWave123Pipeline, Wave63_123_BindingsPipeline) {
    Interpreter interp;
    ...
    // --- Wave 123 REPL bindings (single session) ---

    // 157. control_pidtune_kp plant [1]/[1,1] -> Kp>0 (~1.414)
    ...
    // 158. quantum_bell_state(0) |Phi+> real parts at rows 0 and 3 ~ 0.707107
    expect_ok(interp, "bell = quantum_bell_state(0)");
    ASSERT_GT(interp.state().matrices.count("bell"), 0u);
    const auto& bell = interp.state().matrices.at("bell");
    EXPECT_EQ(bell.rows(), 4u);
    EXPECT_NEAR(bell(0, 0), 0.707107, 1e-4);
    EXPECT_NEAR(bell(3, 0), 0.707107, 1e-4);

    // 159. bzip2_compress_vec / bzip2_decompress_vec roundtrip abracadabra
    ...
    ASSERT_GT(interp.state().matrices.count("bzR"), 0u);
    const auto& bzR = interp.state().matrices.at("bzR");
    EXPECT_EQ(bzR.rows(), 11u);
    EXPECT_NEAR(bzR(0, 0), 97.0, 1e-9);
    EXPECT_NEAR(bzR(10, 0), 97.0, 1e-9);
}
