...
Mesh1D make_mesh(std::size_t n_elements) {
    auto mesh = mesh1d(0.0, 1.0, n_elements);
    if (!mesh.has_value()) {
        return Mesh1D{};
    }
    return *mesh;
...
Mesh2D make_mesh2d(std::size_t nx, std::size_t ny) {
    auto mesh = mesh2d_rectangular(0.0, 0.0, 1.0, 1.0, nx, ny);
    if (!mesh.has_value()) {
        return Mesh2D{};
    }
    return *mesh;
...
Mesh3D make_mesh3d(std::size_t nx, std::size_t ny, std::size_t nz) {
    auto mesh = mesh3d_box(0.0, 0.0, 0.0, 1.0, 1.0, 1.0, nx, ny, nz);
    if (!mesh.has_value()) {
        return Mesh3D{};
    }
    return *mesh;
...
static void BM_FemAssemble1D(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        if (!K.has_value()) {
            state.SkipWithError("assemble_stiffness_1d failed");
            return;
        }
...
static void BM_FemSolve1D(benchmark::State& state) {
    ...
    if (!f_r.has_value()) {
        state.SkipWithError("assemble_load_1d failed");
        return;
    }
    ColMatrix<double> f = *f_r;
    if (auto r = apply_dirichlet(K, f, {0, mesh.nodes.size() - 1}, {0.0, 0.0}); !r) {
        state.SkipWithError("apply_dirichlet failed");
        return;
    }
...
static void BM_FemAssemble2D(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        if (!K.has_value()) {
            state.SkipWithError("assemble_stiffness_2d failed");
            return;
        }
        benchmark::DoNotOptimize(K->data());
    }
...
static void BM_FemAssemble3D(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        if (!K.has_value()) {
            state.SkipWithError("assemble_stiffness_3d failed");
            return;
        }
        benchmark::DoNotOptimize(K->data());
    }
...
static void BM_FemSolve3D(benchmark::State& state) {
    ...
    if (!f_r.has_value()) {
        state.SkipWithError("assemble_load_3d failed");
        return;
    }
    ColMatrix<double> f = *f_r;
