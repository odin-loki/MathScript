...
static void BM_CudaBroadcast(benchmark::State& state) {
    for (auto _ : state) {
        const double result = ms::cuda::broadcast(2.0);
        benchmark::DoNotOptimize(result);
    }
}
...
static void BM_DistLsqr_2x2(benchmark::State& state) {
    auto ctx = init(0, nullptr);
    DMatrix A({{4.0, 1.0}, {1.0, 3.0}});
    DMatrix b({{1.0}, {2.0}});
    ...
    for (auto _ : state) {
        ...
        auto result = dist_lsqr(dA, db, ctx);
        benchmark::DoNotOptimize(result);
    }
    finalize(ctx);
}
BENCHMARK(BM_DistLsqr_2x2);
...
static void BM_CellMemory_Recall(benchmark::State& state) {
    ...
    DMatrix x(8, 1, 1.0);
    benchmark::DoNotOptimize(memory.step(x));
    benchmark::DoNotOptimize(memory.step(x));
    for (auto _ : state) {
...
static void BM_DifModel_Predict_2x1(benchmark::State& state) {
    ...
    DMatrix x(2, 1, 0.5);
    DMatrix y(1, 1, 1.0);
    benchmark::DoNotOptimize(model.update(x, y));
    benchmark::DoNotOptimize(model.update(x, y));
    for (auto _ : state) {
