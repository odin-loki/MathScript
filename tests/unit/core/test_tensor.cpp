...
TEST(TensorTest, reshape_2x3_to_3x2) {
    ...
    const auto& x = *r;
    EXPECT_EQ(x.dims(), 2u);
    EXPECT_EQ(x.size(0), 3u);
    EXPECT_EQ(x.size(1), 2u);
    EXPECT_EQ(x.total_size(), 6u);
    EXPECT_DOUBLE_EQ(x.at(0, 0), 0.0);
    EXPECT_DOUBLE_EQ(x.at(0, 1), 1.0);
    EXPECT_DOUBLE_EQ(x.at(1, 0), 2.0);
    EXPECT_DOUBLE_EQ(x.at(2, 1), 5.0);
}
...
TEST(TensorTest, reshape_2x3_to_6x1) {
    ...
    ASSERT_TRUE(r.has_value());
    const auto& x = *r;
    EXPECT_EQ(x.size(0), 6u);
    EXPECT_EQ(x.size(1), 1u);
    EXPECT_DOUBLE_EQ(x.at(0, 0), 1.0);
    EXPECT_DOUBLE_EQ(x.at(3, 0), 4.0);
    EXPECT_DOUBLE_EQ(x.at(5, 0), 6.0);
}
...
TEST(TensorTest, reshape_2x3_to_1x6) {
    ...
    ASSERT_TRUE(r.has_value());
    const auto& x = *r;
    EXPECT_EQ(x.size(0), 1u);
    EXPECT_EQ(x.size(1), 6u);
    EXPECT_DOUBLE_EQ(x.at(0, 5), 9.0);
}
...
TEST(TensorTest, reshape_preserves_row_major_order) {
    ...
    for (size_t k = 0; k < 6; ++k) {
        EXPECT_DOUBLE_EQ(x.at(k / 2, k % 2), expected[k]);
    }
}
...
TEST(TensorTest, reshape_same_shape_copies_data) {
    ...
    ASSERT_TRUE(r.has_value());
    const auto& x = *r;
    EXPECT_EQ(x.size(0), 2u);
    EXPECT_EQ(x.size(1), 3u);
    EXPECT_DOUBLE_EQ(x.at(0, 1), 42.0);
}

TEST(TensorTest, reshape_invalid_shape_too_small) {
    Tensor<double, 2> t(2, 3);
    const auto r = t.reshape({2u, 2u});
    ASSERT_FALSE(r.has_value());
}

TEST(TensorTest, reshape_invalid_shape_too_large) {
    Tensor<double, 2> t(2, 3);
    const auto r = t.reshape({3u, 3u});
    ASSERT_FALSE(r.has_value());
}

TEST(TensorTest, reshape_invalid_shape_wrong_dims) {
    Tensor<double, 2> t(2, 3);
    const auto r = t.reshape({2u, 2u, 2u});
    ASSERT_FALSE(r.has_value());
}
