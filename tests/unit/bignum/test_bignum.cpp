...
TEST(BigIntBasic, ParseBaseKnown) {
    auto r = BigInt::parse("ff", 16);
    ASSERT_TRUE(r.has_value());
    EXPECT_EQ(r->to_string(), "255");
    r = BigInt::parse("11111111", 2);
    ASSERT_TRUE(r.has_value());
    EXPECT_EQ(r->to_string(), "255");
}

TEST(BigIntBasic, ParseBadBase) {
    const auto r = BigInt::parse("10", 1);
    ASSERT_FALSE(r.has_value());
    EXPECT_TRUE(std::holds_alternative<ms::DomainError>(r.error()));
}

TEST(BigIntBasic, ParseBadDigit) {
    const auto r = BigInt::parse("2", 2);
    ASSERT_FALSE(r.has_value());
    EXPECT_TRUE(std::holds_alternative<ms::DomainError>(r.error()));
}

TEST(BigIntBasic, ConstructBadBaseIsZero) {
    EXPECT_TRUE(BigInt("10", 1).is_zero());
    EXPECT_TRUE(BigInt("2", 2).is_zero());
}
