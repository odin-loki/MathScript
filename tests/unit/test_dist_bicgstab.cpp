...
TEST(DistBiCGSTABTest, stub_single_rank_matches_local_bicgstab) {
    ...
    const auto result = dist_bicgstab(dA, db, ctx).value();
...
TEST(DistBiCGSTABTest, stub_identity_system) {
    auto ctx = init(0, nullptr);
...
TEST(DistBiCGSTABTest, stub_larger_system_matches_local_bicgstab) {
    auto ctx = init(0, nullptr);
...
TEST(DistBiCGSTABTest, dimension_mismatch_errors) {
    auto ctx = init(0, nullptr);
    const ColMatrix<double> A{{2.0, 1.0}, {1.0, 2.0}};
    const ColMatrix<double> b{{1.0}, {0.0}, {0.0}};
    const auto dA = scatter(A, ctx).value();
    const auto db = scatter(b, ctx).value();
    EXPECT_FALSE(dist_bicgstab(dA, db, ctx).has_value());
    finalize(ctx);
}
