...
TEST(ReplInterpAdv_AssignScalar, OverwriteExistingScalar) {
    Interpreter interp;
    ASSERT_TRUE((interp.assign_scalar("a", 1.0)).has_value());
    ASSERT_TRUE((interp.assign_scalar("a", 99.0)).has_value());
    EXPECT_NEAR(interp.state().scalars.at("a"), 99.0, 1e-12);
}

TEST(ReplInterpAdv_AssignScalar, MultipleDistinctScalars) {
    Interpreter interp;
    ASSERT_TRUE((interp.assign_scalar("p", 10.0)).has_value());
    ASSERT_TRUE((interp.assign_scalar("q", 20.0)).has_value());
    ASSERT_TRUE((interp.assign_scalar("r", 30.0)).has_value());
...
TEST(ReplInterpAdv_AssignScalarBinary, VarOperand_UsesStateScalar) {
    Interpreter interp;
    ASSERT_TRUE((interp.assign_scalar("x", 10.0)).has_value());
    ScalarBinaryAssign sa;
    sa.target = "y";
    sa.op = '-';
...
TEST(ReplInterpAdv_AssignScalarBinary, BothVarOperands) {
    Interpreter interp;
    ASSERT_TRUE((interp.assign_scalar("a", 8.0)).has_value());
    ASSERT_TRUE((interp.assign_scalar("b", 2.0)).has_value());
    ScalarBinaryAssign sa;
    sa.target = "c";
    sa.op = '*';
...
TEST(ReplInterpAdv_SessionState, Scalars_PopulatedAfterAssign) {
    Interpreter interp;
    ASSERT_TRUE((interp.assign_scalar("pi_approx", 3.14159)).has_value());
    const auto& s = interp.state();
...
TEST(ReplInterpAdv_SessionState, History_RecordsExecutedLines) {
    ...
    ASSERT_TRUE((interp.execute("y = 6")).has_value());
    const auto& s = interp.state();
...
TEST(ReplInterpAdv_SessionState, History_EmptyLineNotRecorded) {
    Interpreter interp;
    static_cast<void>(interp.execute(""));
    static_cast<void>(interp.execute("   "));
    const auto& s = interp.state();
    EXPECT_TRUE(s.history.empty());
}
...
TEST(ReplInterpAdv_SessionState, Reset_ClearsAll) {
    ...
    ASSERT_TRUE((interp.assign_scalar("a", 1.0)).has_value());
    ASSERT_TRUE((interp.execute("M = [1, 2; 3, 4]")).has_value());
    ASSERT_TRUE((interp.execute("b = 3")).has_value());
    interp.reset();
    const auto& s = interp.state();
    EXPECT_TRUE(s.scalars.empty());
    EXPECT_TRUE(s.matrices.empty());
...
TEST(ReplInterpAdv_SessionState, Plot_FieldInSessionState) {
    Interpreter interp;
    ASSERT_TRUE((interp.execute("plot([5, 10, 15])")).has_value());
    const auto& s = interp.state();
