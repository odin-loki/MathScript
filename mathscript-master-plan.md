...
Waves 1 through 211 incrementally implemented this plan; each wave's specific additions are tracked in full detail in `CHANGELOG.md` rather than duplicated here.
...
- **369 CTest suites, all passing** (as of Wave 211; Waves 206-210 added test cases to existing binaries rather than new CTest registrations, Wave 211 added 1 new suite)
...
- **ms::signal:** sosfilt, advanced IIR/FIR design (cheby1/cheby2/ellip/bessel/remez), spectral analysis (periodogram/music/esprit/pburg/cohere), emd/hht/vmd, modulation family
- **ms::geo:** polygon boolean ops (poly_union/intersect/diff) — robust clipping deferred as higher-risk; convex_hull_3d closed (Wave 193)
- **ms::graph:** planarity/embedding (Boyer-Myrvold), Blossom matching — substantial dedicated efforts
- **ms::finance:** Heston/SABR stochastic-volatility models (Monte Carlo, Markowitz, lookback, Black-Litterman, etc. now closed)
- **ms::bignum:** APFloat/APComplex with full transcendental support — fundamental-rewrite scale
- **ms::image:** SIFT/SURF/ORB, watershed/graph-cut/SLIC, marching-cubes
