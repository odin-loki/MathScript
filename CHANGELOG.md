...
- Coverage/Linux: qualify `ms::memory::aligned_alloc` so it does not clash with POSIX `::aligned_alloc`.
- Bench CI: list `bench_*.cpp` stems instead of grepping the GLOB `add_ms_bench(${_stem} …)` wrapper.
