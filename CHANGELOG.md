...
- Linux Debug CI (coverage/Valgrind/ASan): free runner disk; Ninja link job pool; `-g1` debug info; `-DMS_BUILD_INTEGRATION=OFF` (each integration binary statically copies the whole library and exhausts even 111 GB). Valgrind job timeout 180 min. Full 816 suites remain on Linux GCC Release and Windows.
