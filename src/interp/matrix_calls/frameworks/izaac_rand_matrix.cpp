...
Result<Matrix<double>> handle_izaac_rand_matrix(Interpreter& interp, const MatrixCallAssign& assign) {
    ...
    if (assign.callee == "izaac_rand_matrix" && assign.args.size() == 2) {
        auto rows_val = ctx.parse_scalar_arg(assign.args[0], "izaac_rand_matrix");
        if (!rows_val) {
            return std::unexpected(rows_val.error());
        }
        auto cols_val = ctx.parse_scalar_arg(assign.args[1], "izaac_rand_matrix");
        if (!cols_val) {
            return std::unexpected(cols_val.error());
        }
        auto rows_i = ctx.parse_positive_size_arg(*rows_val, "izaac_rand_matrix", "expected positive integer rows");
        if (!rows_i) {
            return std::unexpected(rows_i.error());
        }
        auto cols_i = ctx.parse_positive_size_arg(*cols_val, "izaac_rand_matrix", "expected positive integer cols");
        if (!cols_i) {
