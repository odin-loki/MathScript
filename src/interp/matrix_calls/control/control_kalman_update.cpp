...
Result<Matrix<double>> handle_control_kalman_update(Interpreter& interp, const MatrixCallAssign& assign) {
    ...
    if (assign.callee == "control_kalman_update" && assign.args.size() == 5) {
...
void ms_register_matrix_call_control_kalman_update() {
    register_matrix_call("control_kalman_update", &handle_control_kalman_update);
}
