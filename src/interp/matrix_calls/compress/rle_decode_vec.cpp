...
Result<Matrix<double>> handle_rle_decode_vec(Interpreter& interp, const MatrixCallAssign& assign) {
    ...
    if (assign.callee == "rle_decode_vec" && assign.args.size() == 1) {
...
void ms_register_matrix_call_rle_decode_vec() {
    register_matrix_call("rle_decode_vec", &handle_rle_decode_vec);
}
