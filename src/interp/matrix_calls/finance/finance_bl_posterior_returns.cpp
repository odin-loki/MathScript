...
Result<Matrix<double>> handle_finance_bl_posterior_returns(Interpreter& interp, const MatrixCallAssign& assign) {
    ...
    if (assign.callee == "finance_bl_posterior_returns" && assign.args.size() == 5) {
...
void ms_register_matrix_call_finance_bl_posterior_returns() {
    register_matrix_call("finance_bl_posterior_returns", &handle_finance_bl_posterior_returns);
}
