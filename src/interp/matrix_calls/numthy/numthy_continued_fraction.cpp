...
Result<Matrix<double>> handle_numthy_continued_fraction(Interpreter& interp, const MatrixCallAssign& assign) {
    ...
    if (assign.callee == "numthy_continued_fraction" && assign.args.size() == 2) {
        double x = 0.0;
        double n_d = 0.0;
...
void ms_register_matrix_call_numthy_continued_fraction() {
    register_matrix_call("numthy_continued_fraction", &handle_numthy_continued_fraction);
}
