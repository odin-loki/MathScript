...
Result<Matrix<double>> handle_imdilate(Interpreter& interp, const MatrixCallAssign& assign) {
    ...
    if ((assign.callee == "imdilate" || assign.callee == "imerode" ||
        ...
        // probe asks for imdilate(ones(512,512), 4999), which is a day. A kernel wider than the image is also
...
void ms_register_matrix_call_imdilate() {
    register_matrix_call("imdilate", &handle_imdilate);
}
