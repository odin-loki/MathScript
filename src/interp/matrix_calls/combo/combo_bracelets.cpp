...
Result<Matrix<double>> handle_combo_bracelets(Interpreter& interp, const MatrixCallAssign& assign) {
    ...
    if (assign.callee == "combo_bracelets" && assign.args.size() == 2) {
        double n_d = 0.0;
...
void ms_register_matrix_call_combo_bracelets() {
    register_matrix_call("combo_bracelets", &handle_combo_bracelets);
}
