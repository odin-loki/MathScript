...
Result<Matrix<double>> handle_ml_gmm_fit(Interpreter& interp, const MatrixCallAssign& assign) {
    using namespace detail;
    MatrixCallCtx ctx(interp);

    Result<Matrix<double>> result =
        std::unexpected(DomainError{"assign", "unsupported matrix call"});
    if (assign.callee == "ml_gmm_fit" &&
        ...
        if (assign.args.size() == 2) {
            auto n_comp = ctx.parse_scalar_arg(assign.args[1], "ml_gmm_fit");
            if (!n_comp) {
...
void ms_register_matrix_call_ml_gmm_fit() {
    register_matrix_call("ml_gmm_fit", &handle_ml_gmm_fit);
}
