...
Result<Matrix<double>> handle_cfd_square_pulse_3d(Interpreter& interp, const MatrixCallAssign& assign) {
    ...
    if (assign.callee == "cfd_square_pulse_3d" &&
        ...
        auto xc = ctx.parse_scalar_arg(assign.args[1], "cfd_square_pulse_3d");
        if (!xc) {
            return std::unexpected(xc.error());
        }
        auto yc = ctx.parse_scalar_arg(assign.args[2], "cfd_square_pulse_3d");
        if (!yc) {
            return std::unexpected(yc.error());
        }
        auto zc = ctx.parse_scalar_arg(assign.args[3], "cfd_square_pulse_3d");
        if (!zc) {
            return std::unexpected(zc.error());
        }
        auto width_x = ctx.parse_scalar_arg(assign.args[4], "cfd_square_pulse_3d");
        if (!width_x) {
            return std::unexpected(width_x.error());
        }
        auto width_y = ctx.parse_scalar_arg(assign.args[5], "cfd_square_pulse_3d");
        if (!width_y) {
            return std::unexpected(width_y.error());
        }
        auto width_z = ctx.parse_scalar_arg(assign.args[6], "cfd_square_pulse_3d");
        if (!width_z) {
            return std::unexpected(width_z.error());
        }
        double amplitude = 1.0;
        if (assign.args.size() == 8) {
            auto amp = ctx.parse_scalar_arg(assign.args[7], "cfd_square_pulse_3d");
            if (!amp) {
