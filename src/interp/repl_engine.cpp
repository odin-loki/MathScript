...
bool is_scalar_expression_rhs(const std::string& rhs) {
    ...
    if (const auto call = parse_scalar_call(text)) {
        ...
        if (fn == "matmul" || fn == "tensorops_matmul" || fn == "tensorops_einsum" ||
            fn == "cuda_add" ||
            fn == "solve" || fn == "dist_solve" || fn == "dist_cg" || fn == "dist_matmul" || fn == "transpose" || fn == "chol" ||
            fn == "det" ||
...
bool is_matrix_call_callee(const std::string& callee) {
    return callee == "matmul" || callee == "tensorops_matmul" || callee == "tensorops_einsum" ||
           callee == "solve" || callee == "dist_solve" || callee == "dist_cg" || callee == "dist_matmul" ||
...
Result<std::string> Interpreter::execute(const std::string& line) {
    ...
    if (lcmd == "help") {
        return std::string{
            ...
            "  lu(A), qr(A), chol(A), solve(A,B), dist_solve(A,B), dist_cg(A,B), dist_matmul(A,B), matmul(A,B), tensorops_matmul(A,B), tensorops_einsum(A,B), cuda_lu(A), cuda_add(A,B), eig_sym(A), svd(A)\n"
