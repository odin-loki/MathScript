...
Result<Matrix<double>> eval_fem_poisson3d(
    ...
    ColMatrix<double> f = fem::assemble_load_3d(mesh, [](double, double, double) { return 1.0; });
...
bool is_scalar_expression_rhs(const std::string& rhs) {
    ...
    if (const auto call = parse_scalar_call(text)) {
        ...
        if (fn == "matmul" || fn == "tensorops_matmul" || fn == "tensorops_einsum" ||
            fn == "cuda_add" ||
            fn == "solve" || fn == "bicgstab" || fn == "dist_solve" || fn == "dist_cg" || fn == "dist_gmres" || fn == "dist_jacobi" || fn == "dist_bicgstab" || fn == "dist_matmul" || fn == "transpose" || fn == "chol" ||
            fn == "det" ||
...
bool is_matrix_call_callee(const std::string& callee) {
    return callee == "matmul" || callee == "tensorops_matmul" || callee == "tensorops_einsum" ||
           callee == "solve" || callee == "bicgstab" || callee == "dist_solve" || callee == "dist_cg" || callee == "dist_gmres" || callee == "dist_jacobi" || callee == "dist_bicgstab" || callee == "dist_matmul" ||
...
bool is_valid_matrix_call_arity(const std::string& callee, size_t arity) {
    if (callee == "matmul" || callee == "tensorops_matmul" || callee == "tensorops_einsum" ||
        ...
        callee == "dist_gmres" || callee == "dist_jacobi" || callee == "dist_bicgstab" || callee == "dist_matmul" || callee == "rand" ||
        callee == "randn" ||
        callee == "kron") {
        return arity == 2;
    }
...
Result<std::string> Interpreter::assign_matrix_call(const MatrixCallAssign& assign) {
    ...
    if (assign.callee == "matmul" && assign.args.size() == 2) {
    ...
    } else if (assign.callee == "bicgstab" && assign.args.size() == 2) {
        auto left = resolve_operand(assign.args[0]);
        if (!left) {
            return std::unexpected(left.error());
        }
    ...
    } else if (assign.callee == "dist_cg" && assign.args.size() == 2) {
        auto left = resolve_operand(assign.args[0]);
        if (!left) {
            return std::unexpected(left.error());
        }
    ...
    } else if (assign.callee == "dist_gmres" && assign.args.size() == 2) {
        auto left = resolve_operand(assign.args[0]);
        if (!left) {
            return std::unexpected(left.error());
        }
    ...
    } else if (assign.callee == "dist_jacobi" && assign.args.size() == 2) {
        auto left = resolve_operand(assign.args[0]);
        if (!left) {
            return std::unexpected(left.error());
        }
        ...
        result = eval_dist_jacobi(*left, *right);
    } else if (assign.callee == "dist_bicgstab" && assign.args.size() == 2) {
        auto left = resolve_operand(assign.args[0]);
        if (!left) {
            return std::unexpected(left.error());
        }
        ...
        result = eval_dist_bicgstab(*left, *right);
...
Result<std::string> Interpreter::assign_scalar_matrix_call(const ScalarMatrixCallAssign& assign) {
    ...
    } else if (assign.callee == "quantum_purity") {
...
Result<std::string> Interpreter::execute(const std::string& line) {
    ...
    if (lcmd == "help") {
        return std::string{
            ...
            "  name = dist_jacobi(A, B) distributed Jacobi (block MPI or gather + jacobi)\n"
            ...
            "  name = sym_mellin(\"expr\",\"t\",\"s\") Mellin transform t-domain to s-domain\n"
            "  name = sym_imellin(\"expr\",\"s\",\"t\") inverse Mellin transform s-domain to t\n"
            "  name = sym_hankel(\"expr\",\"r\",\"k\") Hankel transform r-domain to k-domain\n"
            "  name = sym_ihankel(\"expr\",\"k\",\"r\") inverse Hankel transform k-domain to r\n"
            ...
            "  name = control_poles(num,den) transfer-function poles as Nx2 [re,im]\n"
            "  name = control_zeros(num,den) transfer-function zeros as Nx2 [re,im]\n"
            "  name = control_step_info(num,den) step metrics [rise,settle,overshoot,peak_t,peak_y]\n"
            "  name = control_nyquist(num,den) Nyquist locus as Nx2 [Re H, Im H]\n"
            ...
            "  name = finance_geo_asian_call(S,K,T,r,sigma,n_fixings) geometric Asian call price\n"
            "  name = finance_geo_asian_put(S,K,T,r,sigma,n_fixings) geometric Asian put price\n"
            ...
            "  lu(A), qr(A), chol(A), solve(A,B), bicgstab(A,B), dist_solve(A,B), dist_cg(A,B), dist_gmres(A,B), dist_jacobi(A,B), dist_bicgstab(A,B), dist_matmul(A,B), matmul(A,B), tensorops_matmul(A,B), tensorops_einsum(A,B), cuda_lu(A), cuda_add(A,B), eig_sym(A), svd(A)\n"
    ...
    if (eq != std::string::npos) {
        ...
        if (try_parse_matrix_scalar_mixed_call_assignment(cmd, matrix_scalar_call)) {
            ...
            if (matrix_scalar_call.callee == "graph_bipartite_match") {
                ...
                value = eval_graph_bipartite_match(*matrix, left_size);
            }
        ...
        if (try_parse_matrix_dual_matrix_call_assignment(cmd, matrix_dual_call)) {
            ...
            if (matrix_dual_call.callee == "control_poles") {
                auto value = eval_control_poles(*arg_a_m, *arg_b_m);
                if (!value) {
                    return std::unexpected(value.error());
                }
                state_.matrices[matrix_dual_call.target] = *value;
            ...
            if (matrix_dual_call.callee == "control_zeros") {
                auto value = eval_control_zeros(*arg_a_m, *arg_b_m);
                if (!value) {
                    return std::unexpected(value.error());
                }
                state_.matrices[matrix_dual_call.target] = *value;
            ...
            if (matrix_dual_call.callee == "control_step_info") {
                auto value = eval_control_step_info(*arg_a_m, *arg_b_m);
                if (!value) {
                    return std::unexpected(value.error());
                }
                state_.matrices[matrix_dual_call.target] = *value;
            ...
            if (matrix_dual_call.callee == "control_nyquist") {
                auto value = eval_control_nyquist(*arg_a_m, *arg_b_m);
                if (!value) {
                    return std::unexpected(value.error());
                }
                state_.matrices[matrix_dual_call.target] = *value;
        ...
        if (const auto open = rhs.find('('); open != std::string::npos && !rhs.empty() &&
            ...
            if (callee == "finance_geo_asian_call") {
                const auto call_args = split_call_args(rhs);
                if (!call_args || call_args->size() != 6) {
                    return std::unexpected(DomainError{
                        "finance_geo_asian_call",
                ...
                double r = 0.0;
                double sigma = 0.0;
                double n_fixings_d = 0.0;
                if (!parse_number(trim_copy((*call_args)[0]), S) ||
                    !parse_number(trim_copy((*call_args)[1]), K) ||
                    !parse_number(trim_copy((*call_args)[2]), T) ||
                    !parse_number(trim_copy((*call_args)[3]), r) ||
                    !parse_number(trim_copy((*call_args)[4]), sigma) ||
                    !parse_number(trim_copy((*call_args)[5]), n_fixings_d)) {
                    return std::unexpected(DomainError{
                        "finance_geo_asian_call",
                        "expected finance_geo_asian_call(S,K,T,r,sigma,n_fixings)"});
                }
                ...
                return assign_scalar(lhs,
                                     finance::geo_asian_call(S, K, T, r, sigma, n_fixings));
            }
            if (callee == "finance_geo_asian_put") {
                const auto call_args = split_call_args(rhs);
                if (!call_args || call_args->size() != 6) {
                    return std::unexpected(DomainError{
                ...
                return assign_scalar(lhs, finance::geo_asian_put(S, K, T, r, sigma, n_fixings));
            }
            ...
            if (callee == "quantum_schmidt_rank") {
                const auto call_args = split_call_args(rhs);
                if (!call_args || call_args->size() != 3) {
                    return std::unexpected(DomainError{
            ...
            if (callee == "quantum_uncertainty") {
                const auto call_args = split_call_args(rhs);
                if (!call_args || call_args->size() != 3) {
                    return std::unexpected(DomainError{
                        "quantum_uncertainty", "expected quantum_uncertainty(psi, A, B)"});
                }
                auto psi_m = eval_matrix_operand(trim_copy((*call_args)[0]));
                if (!psi_m) {
                    return std::unexpected(psi_m.error());
                }
                auto A_m = eval_matrix_operand(trim_copy((*call_args)[1]));
                if (!A_m) {
                    return std::unexpected(A_m.error());
                }
                auto B_m = eval_matrix_operand(trim_copy((*call_args)[2]));
                if (!B_m) {
                    return std::unexpected(B_m.error());
                }
                auto value = eval_quantum_uncertainty(*psi_m, *A_m, *B_m);
                if (!value) {
                    return std::unexpected(value.error());
                }
                return assign_scalar(lhs, *value);
            ...
            if (callee == "finance_max_sharpe_portfolio") {
                const auto call_args = split_call_args(rhs);
                if (!call_args || call_args->size() != 3) {
                    return std::unexpected(DomainError{
                        "finance_max_sharpe_portfolio",
                        "expected finance_max_sharpe_portfolio(cov, mu, risk_free)"});
                }
                auto cov_m = eval_matrix_operand(trim_copy((*call_args)[0]));
                if (!cov_m) {
                    return std::unexpected(cov_m.error());
                }
                auto mu_m = eval_matrix_operand(trim_copy((*call_args)[1]));
                if (!mu_m) {
                    return std::unexpected(mu_m.error());
                }
                double risk_free = 0.0;
                if (!parse_number(trim_copy((*call_args)[2]), risk_free)) {
                    auto rf_expr = eval_scalar_expr(state_, trim_copy((*call_args)[2]));
                    if (!rf_expr) {
                        return std::unexpected(rf_expr.error());
                    }
                    risk_free = *rf_expr;
                }
                auto w = eval_finance_max_sharpe_portfolio(*cov_m, *mu_m, risk_free);
                if (!w) {
                    return std::unexpected(w.error());
                }
    ...
    if (const auto open_paren = cmd.find('('); open_paren != std::string::npos && !cmd.empty() &&
        ...
        if (fn == "finance_max_sharpe_portfolio") {
            ...
            if (!call_args || call_args->size() != 3) {
                return std::unexpected(DomainError{
                    fn, "expected finance_max_sharpe_portfolio(cov, mu, risk_free)"});
            }
            auto cov_m = eval_matrix_operand(trim_copy(call_args->at(0)));
            if (!cov_m) {
                return std::unexpected(cov_m.error());
            }
            auto mu_m = eval_matrix_operand(trim_copy(call_args->at(1)));
            if (!mu_m) {
                return std::unexpected(mu_m.error());
            }
            double risk_free = 0.0;
            if (!parse_number(trim_copy(call_args->at(2)), risk_free)) {
                auto rf_expr = eval_scalar_expr(state_, trim_copy(call_args->at(2)));
                if (!rf_expr) {
                    return std::unexpected(rf_expr.error());
                }
                risk_free = *rf_expr;
    ...
    if (std::regex_match(cmd, match, senary)) {
        ...
        if (fn == "finance_geo_asian_call") {
            ...
            if (!parse_number(trim(match[2].str()), S) || !parse_number(trim(match[3].str()), K) ||
                !parse_number(trim(match[4].str()), T) || !parse_number(trim(match[5].str()), r) ||
                !parse_number(trim(match[6].str()), sigma) ||
                !parse_number(trim(match[7].str()), n_fixings_d)) {
                return std::unexpected(DomainError{
                    "finance_geo_asian_call",
                    "expected finance_geo_asian_call(S,K,T,r,sigma,n_fixings)"});
            }
    ...
    if (std::regex_match(cmd, match, ternary)) {
        ...
        fn == "sym_hankel" || fn == "sym_ihankel" ||
            ...
            } else if (fn == "sym_mellin") {
                value = eval_sym_transform_strings(arg_a, arg_b, arg_c, fn.c_str(), sym_mellin);
            } else if (fn == "sym_imellin") {
                value = eval_sym_transform_strings(arg_a, arg_b, arg_c, fn.c_str(), sym_imellin);
            } else if (fn == "sym_hankel") {
                value = eval_sym_transform_strings(arg_a, arg_b, arg_c, fn.c_str(), sym_hankel);
            } else if (fn == "sym_ihankel") {
                value = eval_sym_transform_strings(arg_a, arg_b, arg_c, fn.c_str(), sym_ihankel);
        ...
        if (fn == "quantum_uncertainty") {
            ...
            auto psi_m = resolve_arg(trim_copy((*call_args)[0]));
            if (!psi_m) {
                return std::unexpected(psi_m.error());
            }
            auto A_m = resolve_arg(trim_copy((*call_args)[1]));
            if (!A_m) {
                return std::unexpected(A_m.error());
            }
            auto B_m = resolve_arg(trim_copy((*call_args)[2]));
            if (!B_m) {
                return std::unexpected(B_m.error());
            }
            auto value = eval_quantum_uncertainty(*psi_m, *A_m, *B_m);
        ...
        if (fn == "geo_bezier_eval_x" || fn == "geo_bezier_eval_y" || fn == "bwt_decode_vec" ||
            ...
            if (call_args && call_args->size() == 2) {
                ...
                if (fn == "graph_bipartite_match") {
                    ...
                    auto value = eval_graph_bipartite_match(*ctrl, left_size);
                    if (!value) {
                        return std::unexpected(value.error());
                    }
                    return std::to_string(*value) + "\n";
    ...
    if (std::regex_match(cmd, match, binary)) {
        ...
        if (fn == "quantum_grover_optimal_iterations") {
            ...
            double n_marked_d = 0.0;
            if (!parse_number(arg_a, n_qubits_d) || !parse_number(arg_b, n_marked_d)) {
                return std::unexpected(DomainError{
                    "quantum_grover_optimal_iterations",
        ...
        if (fn == "plot" || fn == "scatter" || fn == "solve" || fn == "bicgstab" || fn == "dist_solve" ||
            fn == "dist_cg" || fn == "dist_gmres" || fn == "dist_jacobi" || fn == "dist_bicgstab" || fn == "dist_matmul" ||
    ...
    if (std::regex_match(cmd, match, unary)) {
        ...
        } else if (fn == "graph_eulerian_path") {
            auto path = eval_graph_eulerian_path(*matrix);
            if (!path) {
                return std::unexpected(path.error());
            }
        ...
        } else if (fn == "graph_louvain") {
            auto comms = eval_graph_louvain(*matrix);
            if (!comms) {
                return std::unexpected(comms.error());
            }
            out << "louvain =\n";
            print_matrix(out, *comms);
        } else if (fn == "graph_eigenvector_centrality") {
            auto ec = eval_graph_eigenvector_centrality(*matrix);
            if (!ec) {
                return std::unexpected(ec.error());
            }
