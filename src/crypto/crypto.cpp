...
std::vector<uint8_t> hkdf_sha512(std::span<const uint8_t> ikm,
    ...
    if (out_len > 255 * sha512_digest_size) {
        return {};
    }

    std::vector<uint8_t> extract_salt(salt.begin(), salt.end());
    if (extract_salt.empty()) {
        extract_salt.assign(sha512_digest_size, 0);
    }
    const std::vector<uint8_t> prk = hmac_sha512(extract_salt, ikm);

    const std::size_t blocks = (out_len + sha512_digest_size - 1) / sha512_digest_size;
    std::vector<uint8_t> okm;
    okm.reserve(out_len);
    ...
    for (std::size_t i = 1; i <= blocks; ++i) {
        ...
        t_prev = hmac_sha512(prk, hmac_input);
        const std::size_t take = std::min(sha512_digest_size, out_len - okm.size());
        okm.insert(okm.end(), t_prev.begin(), t_prev.begin() + static_cast<std::ptrdiff_t>(take));
    }
    return okm;
}
...
std::string hmac_sha512_hex(std::span<const uint8_t> key,
                            std::span<const uint8_t> data) {
    return to_hex(hmac_sha512(key, data));
}
