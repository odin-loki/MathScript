...
| 5. Fuzz, 24 h × 7 | `fuzz-24h.yml` | dispatched 2026-09-16, **failed** | not in the library: `fuzz_repl_input` hit the RSS cap at 69 minutes with a **43.8 MB live heap**, and the corpus flag was a no-op so no Actions run had ever been seeded. Both fixed in `scripts/fuzz_session.sh` — see criterion 5 — and **the marathon needs re-dispatching**. A local run started the same evening predates the fix and will hit the same cap. The smoke is clean (7 targets × 4096 runs, no crash) though it was not seeded either |
