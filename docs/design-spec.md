# MathScript — original design specification

This is the original design specification (modules, conventions, phased delivery). It is **not** the current product outline. For the tree as it exists now, start at [`README.md`](../README.md). Wave-by-wave implementation history is [`WAVES.md`](WAVES.md).
...
- ~92% line coverage (90% tag gate, 80% CI gate); AddressSanitizer + UBSan; libFuzzer smoke (7 targets)
- **873 CTest suites, all passing** on Linux GCC 13 Release with CUDA off (the catalogue that `docs/RELEASE.md` gates on; other configurations enable or exclude different suites)
...
- Wave 223: Performance pass V — explicit fft2 dims, 2D FFT conv2, watershed/blur opts, welch/filtfilt/resample buffer reuse, baseline schema + 7-bench smoke
- Wave 224: Performance pass VI (final) — medfilt2, boxfilter, imresize, bilateral, Dijkstra, stats quantile, 22-bench smoke, MSVC baseline script
- Wave 225: Performance pass VII (closure) — tensorops einsum, imfilter/sharpen, Floyd-Warshall, CZT/hilbert buffers, 23-bench smoke; profiling iteration complete
