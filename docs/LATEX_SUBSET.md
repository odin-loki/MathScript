...
* `Head::Real` **iff** `ms::format_exact(value)` contains a `.` or an `e` — that is the whole condition, and it is not "below 1e6": `format_exact(1234567.0)` is `1234567`, bare digits, and falls under N1 like any other whole-valued Real. Covers `2.5`, `0.3333333333333333`, `0.0001`, `1e20`, `1e-7`, `1e6` (which is already `1 \times 10^{6}`, verified), and every denormal.
...
| N13 | any Greek spelling that is not exactly canonical: `ALPHA`, `aLPHA`, `GaMmA`, `GAMMA` | `\mathrm{A}`, `\alpha`, `\Gamma`, `\Gamma` | `symbol("Alpha")`, `symbol("alpha")`, `symbol("Gamma")`, `symbol("Gamma")` | `greek_letter` lowercases the whole name to match and then keys `capital` on `base[0]` alone (notation.cpp:373-385), so `ALPHA` is the capital -- and `alpha` has no `\Alpha` control sequence, so the capital falls through to the upright letter. This row said `ALPHA` prints `\alpha`; it does not, and the code is what decided. |
