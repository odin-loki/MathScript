...
**374 CTest suites** as of Wave 227 (CHANGELOG); profiling passes 218–227 complete; **27** benchmark executables in smoke. Individual suites contain many `TEST()` cases (several thousand total across the tree). Tests link the `mathscript` INTERFACE library and `GTest::gtest_main` unless noted.
